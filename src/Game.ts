...
class Game {
    ...
    //敌机血量
    private hps: Array<number> = [1, 2, 3];
    // 敌机速度
    ...
    onLoop(): void {
        // 遍历舞台上所有的飞机,更改其状态
        for (let i: number = this.roleBox.numChildren - 1; i > -1; i--) {
            //此处用var，该如何？需要去了解ts
            let role: Role = this.roleBox.getChildAt(i) as Role;
            if (role && role.speed) {
                // 根据飞机的速度改变位置
                role.y += role.speed;
                // 敌机移动到显示区域外,就移除掉
            ...
            if (role.shootType > 0) {
                ...
                if (time > role.shootTime) {
                    ...
                    let pos: Array<number> = this.bulletPos[role.shootType - 1];
        ...
        for (let i: number = this.roleBox.numChildren - 1; i > -1; i--) {
            //获取对象1
            let role1: Role = this.roleBox.getChildAt(i) as Role;
            if (role1.hp < 1) continue;
            for (let j: number = i - 1; j > 0; j--) {

                let role2: Role = this.roleBox.getChildAt(j) as Role;
                if (!role2.visible) continue;
                //如果角色未死亡，并且阵营不同才进行碰撞
                if (role2.hp > 0 && role1.camp != role2.camp) {
                    //计算碰撞区域
                    let hitRadius: number = role1.hitRadius + role2.hitRadius;
                    //根据距离判断是否怕碰撞
        ...
        // 关卡越高，创建敌机的间隔越短
        let cutTime: number = this.level < 30 ? this.level * 2 : 60;
        //关卡越高，敌机的速度越快
        let speedUp: number = Math.floor(this.level / 6);
        // 关卡越高，敌机的血量越高
        let hpUp: number = Math.floor(this.level / 8);
        // 关卡越高，敌机的数量越多
        let numUp: number = Math.floor(this.level / 10);
    ...
    lostHp(role: Role, lostHp: number): void {
        ...
        if (role.hp > 0) {
            ...
            if (role.heroType > 0) {
                ...
                if (role.type === "enemy3") {
                    //随机子弹？或者血瓶
                    let type: number = Math.random() < 0.6 ? 2 : 3;
                    let item: Role = Laya.Pool.getItemByClass('role', Role);
    ...
    createEnmy(type: number, num: number, speed: number, hp: number): void {
